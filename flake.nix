  ...
  outputs =
    ...
    let
      base = flake-utils.lib.eachDefaultSystem (system: {
        ...
        constants = rec {
          default-packages = (import ./util/util.nix { }).create-default-packages {
      ...
      intel-packages =
        { pkgs, ... }:
        {
          ...
          environment.systemPackages = (
            builtins.filter (x: x != pkgs.azure-cli) base.constants.x86_64-darwin.default-packages
          );
        };
      arm-packages =
        { pkgs, ... }:
        {
          environment.systemPackages = base.constants.aarch64-darwin.default-packages;
        };
    in
    let
      myLib = import ./lib/lib.nix { inherit darwin intel-packages arm-packages; };
    in
    let
      base-intel = (import ./darwin/bootstrap.nix { hostPlatform = "x86_64-darwin"; });
      base-arm = (import ./darwin/bootstrap.nix { hostPlatform = "aarch64-darwin"; });
      ...
      ci-arm-packages =
        { pkgs, ... }:
        {
          ...
          environment.systemPackages = (
            builtins.filter (
              x: x != legacyPackages.aarch64-darwin.mysql-workbench-dist && (x != pkgs.tailspin)
      ...
      dotfiles =
        ...
        lib.makeOverridable
          (
            ...
            { pkgs, ... }:
            {
              system.activationScripts.postActivation.text = myLib.configure-environment {
                inherit
                  primaryUser
                  home
                  pkgs
          ...
          )
          {
            ...
            withVSCodeSettings = (lib.fileContents ./config/vscode/${primaryUser}/settings.json);
            withShell = "";
            withDotFiles = [ ];
          };
    in
